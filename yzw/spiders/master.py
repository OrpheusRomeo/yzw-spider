...
class MasterSpider(scrapy.Spider):
    ...
    def start_requests(self):
        keywords = self.get_universities()
        for i in keywords:
    ...
    def call_detail(self, response):
        ...
        item["Subject1"] = self.trans_data(Subject1).strip()
        Subject2 = response.xpath("//div[@class='zsml-result']/table/tbody/tr[1]/td[2]/text()").extract()
        item["Subject2"] = self.trans_data(Subject2).strip()
        Subject3 = response.xpath("//div[@class='zsml-result']/table/tbody/tr[1]/td[3]/text()").extract()
        item["Subject3"] = self.trans_data(Subject3).strip()
        Subject4 = response.xpath("//div[@class='zsml-result']/table/tbody/tr[1]/td[4]/text()").extract()
        item["Subject4"] = self.trans_data(Subject4).strip()
        item["University"] = response.xpath("//table[@class='zsml-condition']/tbody/tr[1]/td[2]/text()").extract_first().strip()
        item["ExamType"] = response.xpath("//table[@class='zsml-condition']//tbody/tr[1]/td[4]/text()").extract_first().strip()
        item["College"] = response.xpath("//table[@class='zsml-condition']//tbody/tr[2]/td[2]/text()").extract_first().strip()
        item["Major"] = response.xpath("//table[@class='zsml-condition']//tbody/tr[2]/td[4]/text()").extract_first().strip()
        item["StudyType"] = response.xpath("//table[@class='zsml-condition']//tbody/tr[3]/td[2]/text()").extract_first().strip()
        item["ResearchInterests"] = response.xpath("//table[@class='zsml-condition']//tbody/tr[3]/td[4]/text()").extract_first().strip()
        item["Teacher"] = response.xpath("//table[@class='zsml-condition']//tbody/tr[4]/td[2]/text()").extract_first().strip()
        item["StudentNo"] = response.xpath("//table[@class='zsml-condition']//tbody/tr[4]/td[4]/text()").extract_first().strip()
        item["Content"] = response.xpath("//table[@class='zsml-condition']//tbody/tr[5]/td[1]/text()").extract_first().strip()
    ...
    def get_universities(self):
        universities = [ '南京大学','南开大学','外交学院','武汉大学','华中科技大学', 
                        '清华大学', '北京大学', '中科院大学', '浙江大学', '复旦大学', 
                        '上海交通大学','中国科学技术大学', '哈尔滨工业大学', '西安交通大学']
        universities = ['南京大学', '南开大学']
        return universities
